<script lang="ts">
  ...
  $: promotionId = $page.params.promotionId;
  ...
  async function loadData() {
    ...
    try {
      ...
      formData = {
        ...
        start_date: promotion.start_date ? formatDateTimeLocal(promotion.start_date) : '',
        end_date: promotion.end_date ? formatDateTimeLocal(promotion.end_date) : '',
  ...
  async function handleSubmit() {
    ...
    try {
      ...
      if (result.success) {
        ...
        error = result.error || 'Failed to update promotion';
      }
    } catch (err) {
...
<svelte:head>
  <title>Edit Promotion - {sponsor?.name || 'Admin'}</title>
</svelte:head>
